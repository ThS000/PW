...
function renderGpuList() {
    ...
    renderGpuList.addEventListener('Click', () => {
        const li = document.createAttribute('li')
        li.textContent = gpuList
        gpuList.appendChild(li);
    }
)
  
  renderGpuList(gpuList.push);

   if( gpus.filter(value => value < 1 )){
    <p class="empty-state">Nenhuma GPU cadastrada.</p>
   }

   classlist.addEventListener 
   dispatchEvent.classlist.appendChild('card')
